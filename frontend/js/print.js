...
function toSeedString(seed) {
    return ("0".repeat(15) + seed.toString(16)).slice(-16);
}
async function loadSeedString(seed) {
...
window.addEventListener("load", () => {
    const printsDataElm = document.getElementById("prints_data");
    ...
    elm.getElementsByClassName("close_button")[0].addEventListener("click", () => {
        elm.remove();
    });
    const createNewPageBtn = document.getElementById("create_new_page");
    const newPageBtnRow = document.getElementById("new_page_btn_row");
    ...
    async function generate() {
        let newPageCheckElm = document.getElementById("repage_flg");
        let newPageFlg = false;
        if ("checked" in newPageCheckElm) {
            newPageFlg = newPageCheckElm.checked;
        }
        let seedsElm = Array.from(document.getElementsByClassName("seed_input_elm"));
        let html = "";
        for (let i of seedsElm) {
            ...
            for (let i = 0; i < 10; ++i) {
                ...
                problems.push(new Date((endDate.getTime() - startDate.getTime()) * randVal +
                    startDate.getTime()));
            }
            html += `
                ...
                return `
                            <tr>
                                ...
                                <td class="date">${value.getFullYear()}/${("0" + value.getMonth()).slice(-2)}/${("0" + value.getDate()).slice(-2)}</td>
                                <td>Sun</td>
                                <td>Mon</td>
                                <td>Tue</td>
                                <td>Wed</td>
