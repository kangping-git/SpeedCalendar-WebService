...
document.addEventListener("DOMContentLoaded", () => {
    window.submit_twitter = () => {
        ...
        window.open(
            "https://twitter.com/intent/tweet?text=" +
                ...
                time_to_string(time) +
                " (" +
                (document.getElementById("weeksta") as HTMLSelectElement).value ===
                "startmon"
                ...
                : "Week start on Sunday" +
                      ...
                      " " +
                      (document.getElementById("icon_telephone") as HTMLInputElement).value +
    ...
    window.submit_result = () => {
        if ((document.getElementById("icon_telephone") as HTMLInputElement).value.bytes() < 200) {
            fetch("/play/games/classic/submit", {
                method: "POST",
                body: JSON.stringify({
                    type: ["long", "mid", "short", "longlong", "era", "era2"].indexOf((document.getElementById("targetmode") as HTMLInputElement).value),
                    time: time,
                    score: score,
                    comment: (document.getElementById("icon_telephone") as HTMLInputElement).value,
                }),
            });
            window.key_down(9);
        } else {
    ...
    let html = "";
    let start_bool: boolean | null = false;
    ...
    function date_to_string(ms: Date): string {
        if (
            (document.getElementById("targetmode") as HTMLSelectElement).value === "era" ||
            (document.getElementById("targetmode") as HTMLSelectElement).value === "era2"
        ) {
    ...
    function new_question() {
        ...
        const targetMode = (document.getElementById("targetmode") as HTMLSelectElement).value;
        const now = new Date();
        let mondai1 =
            targetMode === "long"
                ? new Date(2582, 9, 15)
    ...
    function tick() {
        ...
        if (start_bool === true) {
            ...
            if ((document.getElementById("displaymode") as HTMLSelectElement).value === "concentration") {
    ...
    window.key_down = (key: number) => {
        if (start_bool === true) {
            if (keys.includes(key)) {
                document.getElementById("answer-" + mon)!.innerText = yobis[key];
                if ((document.getElementById("weeksta") as HTMLSelectElement).value === "startsun") {
                    key = (key - 1 + 7) % 7;
                }
    ...
    function updateDayString() {
        ...
        if ((document.getElementById("weeksta") as HTMLInputElement).value == "startsun") {
            yobis.unshift(yobis[6]);
            yobis.pop();
        }
        (document.getElementById("button-0") as HTMLElement).innerText = yobis[0];
        (document.getElementById("button-1") as HTMLElement).innerText = yobis[1];
        (document.getElementById("button-2") as HTMLElement).innerText = yobis[2];
        (document.getElementById("button-3") as HTMLElement).innerText = yobis[3];
        (document.getElementById("button-4") as HTMLElement).innerText = yobis[4];
        (document.getElementById("button-5") as HTMLElement).innerText = yobis[5];
        (document.getElementById("button-6") as HTMLElement).innerText = yobis[6];
